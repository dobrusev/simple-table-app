...
class BarChart {
	...
	getElementPosition ( index, mockWidth ) {
		let position = this.barMargin;
		for ( let i = 0; i < index; i++ )
			position += mockWidth[ i ];
		return position;
	}
	...
	renderBar ( analytic, index ) {
		...
		let mockWidth = [ this.rect.width * 0.40, this.rect.width * 0.20, this.rect.width * 0.15, this.rect.width * 0.25 ];
		let percentageValue = ChartUtil.toPercentage( value, this.maxPropValue, this.maxMaxDataHeight );
		let position = this.getElementPosition( index, mockWidth );

		return RenderUtil.createRectangle( position, mockWidth[ index ], percentageValue, value );
	}
	...
	renderAxisTitle ( analytic, index ) {
		...
		let mockWidth = [ this.rect.width * 0.40, this.rect.width * 0.20, this.rect.width * 0.15, this.rect.width * 0.25 ];
		let positon = this.getElementPosition( index, mockWidth );
		RenderUtil.setElementStyle( el, [ {
			"style": "width: " + this.barWidth + "px; left:"
				+ positon + "px;"
		} ] );
